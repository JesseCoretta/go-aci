...
var strInSliceMap map[int][]bool = map[int][]bool{
...
func TestStrInSlice(t *testing.T) {
	for i, values := range [][]string{
		{`candidate1`, `blargetty`, `candidate7`, `squatcobbler`, `<censored>`},
		{`Ó-aîï4Åø´øH«w%);<wÃ¯`, `piles`, `4378295fmitty`, string(rune(0)), `broccoli`},
	} {
		for j, val := range values {
			result_expected := strInSliceMap[i][j]
			...
			result_received := strInSlice(val, values)
